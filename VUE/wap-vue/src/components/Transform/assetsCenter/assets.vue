<template>
    ...
    <div class="pl-8 pr-8 total_wrap">
      <div class="flex justify-between text-grey relative">
        <div class="flex items-center">
          <span class="text-grey text-30">{{ $t("总资产") }}</span>
  ...
  : " --"
    ...
    <div class="pl-8 pr-8">
      <div class="assets-content-title text-34 titleColor">
        {{ $t("投资组合") }}
      </div>
      <div class="assets-list text-30" v-for="(item, index) in list" :key="index">
        ...
        <div class="flex justify-between mt-10 pl-3">
          <div class="flex-col text-30" v-if="!eyetel">
          ...
          <div class="flex-col text-30" v-else>
            <div class="text-grey">{{ $t("可用") }}</div>
