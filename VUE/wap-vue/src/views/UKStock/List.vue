        ...
        <section class="etf-container">
          ...
          <section class="news-banner-container">
            <van-swipe class="swipe-box" :autoplay="5000" indicator-color="white" v-if="isZh">
              <van-swipe-item>
                <img src="@/assets/image/ukStock/uk1-zh.png" alt="" />
              </van-swipe-item>
              <van-swipe-item>
                <img src="@/assets/image/ukStock/uk2-zh.png" alt="" />
              </van-swipe-item>
            </van-swipe>
            <van-swipe class="swipe-box" :autoplay="5000" indicator-color="white" v-else>
              <van-swipe-item>
                <img src="@/assets/image/ukStock/uk1-en.png" alt="" />
              </van-swipe-item>
              <van-swipe-item>
                <img src="@/assets/image/ukStock/uk2-en.png" alt="" />
              </van-swipe-item>
            </van-swipe>
          </section>
          ...
          <div class="divider mt-4"></div>
          <div class="nav">
            <en-stock-ex-nav :prominentListData="prominentListData" />
          </div>
          <div class="all-etf-ranking">
            <div class="etf-table">
              <ul>
                <li class="li-title">
                  <p class="li-items">{{ t('名称代码') }}</p>
                  <p class="li-items">{{ t('SparkDiagram') }}</p>
                  <p class="li-items">{{ t('uptodate') }}</p>
                  <p class="li-items">{{ t('涨幅') }}</p>
                  <p class="li-items">{{ t('金额') }}</p>
                </li>
                <li v-for="(item ) in defaultListData.slice(0, 5)" :key="item.symbol" @click="itemClick(item)">
                  <div class="li-items">
                    <p>{{ item.name }}</p>
                    <p class="gray-text">
                      {{ item.symbol }}
                  ...
                  <div class="li-items" style="width: 100px;">
                    <m-echarts :dataObj="item" :ratio="item.changeRatio" :index="item.symbol + 2" />
                  </div>
                  <div class="li-items">
            ...
            <div class="more-box" @click="moreOpen(0, t('英股交易'))">
...
import enStockExNav from "@/components/trade/en-stock-ex-nav/index.vue";
...
const itemClick = (item) => {
  router.push(
    `/quotes/usStockDetail?symbol=${item.symbol}&symbolType=UK-stocks&enName=${item.enName}&tabIndex=${props.tabActive}`
  );
};
//打开更多
const moreOpen = (index, name) => {
  router.push(`/quotes/UsStockMore?index=${index}&typName=${name}&symbolType=UK-stocks&tabIndex=${props.tabActive}`);
};
...
.container-box {
  ...
  .etf-container {
    ...
    .indicator-container {
      ...
      .up-bg {
        background: linear-gradient(180deg, $en-hot-bg-1 0%, $en-hot-bg-2 100%);
      }
