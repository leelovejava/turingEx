...
const recordPath = `/ipo/stock?type=record&value=stock&stock=${stockType}&stockActive=${stockActive}`
...
const drawLots = () => {
  ...
  let params = {
    amount: amount.value,
    code: listData.value.productCode,
  }
  drawLotsApply(params).then((res) => {
    showToast(t('抽签成功'));
