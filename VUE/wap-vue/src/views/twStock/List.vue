  ...
  <div class="foreign-container-box">
    ...
    <div class="pt-5 px-6">
      <van-notice-bar class="text-26 textColor" left-icon="" :scrollable="false" background="transparent">
        <template #left-icon slot="left-icon" class=" flex items-center more-img"><img class="w-10 h-10  more-img"
    ...
    <section class="hots-container">
      <div class="hots-symbol flex">
        <div class="hots-symbol-item" v-for="(item, index) in indexCardInfo" :key="item.symbol" @click="itemClick(item)">
          <p class="name">
    ...
    <!-- 快捷 -->
    <section class="nav">
      <tw-stock-ex-nav :defaultEtfListData="defaultListData" />
    </section>
    ...
    <div class="hot-swiper">
      ...
      <div class="container">
        <van-swipe class="swipe-box" :autoplay="5000" indicator-color="white">
          <van-swipe-item v-for="(item, index) in getSwiperList(indexCardInfo, 6, 3)">
            <div class="swipe-item">
              <div v-for="(items, i) in item" :key="items.uuid" class="child" @click="itemClick(items)">
                ...
                <div class="mid">
                  <m-echarts :dataObj="items" :ratio="items.changeRatio" :index="items.symbol + 2" />
                </div>
            ...
            <div class="etf-table">
              <ul class="">
                ...
                <li class="flex quotesItem" v-for="(item, index) in defaultListData.slice(0, 5)" :key="index"
                  ...
                  <div class="item4 text-center">
                    <p><m-echarts :dataObj="item" :ratio="item.changeRatio" :index="item.symbol + 3" /></p>
                  </div>
                  <div class="item5 text-right">
...
const itemClick = (item) => {
  router.push(`/quotes/detail?symbol=${item.symbol}&type=TW-stocks&symbolType=TW-stocks&tabIndex=${props.tabActive}`)
  // router.push(`/quotes/usStockDetail?symbol=${item.symbol}&symbolType=TW-stocks&enName=${item.enName}`)
}

const moreOpen = (index, name) => {
  router.push(`/quotes/hotModules?index=${index}&typName=${name}&symbolType=TW-stocks&tabIndex=${props.tabActive}`)
