      ...
      <div class="all-etf-ranking">
        <div class="etf-table">
          <ul>
            ...
            <li v-for="(item) in entrustList" :key="item.symbol" @click="itemClick(item)">
              <div class="flex-r">
                <div class="flex-r-item">
                  <p>{{ item.symbol }}</p>
                  <p class="gray-text">{{ item.create_time ? item.create_time.slice(11) : '-' }}</p>
                </div>
                <div class="flex-r-item">
                  <p :class="item.price < 1 ? 'text-up' : 'text-down'">{{
                    item.price
                  }}</p>
                </div>
                <div class="flex-r-item">
...
const getTime = (type) => {
  ...
  let start = dayjs().add(t, 'd').format('YYYY-MM-DD') // 默认最近7天
  if (type === 2) {
    start = props.startTime
    end = props.endTime
  }
  console.log(t, start, end, 'start, end')
  return {
    start,
    end
  }
...
const getOrderList = () => {
  let { start, end } = getTime(props.tabActive)
