    ...
    <div class="container-box">
      ...
      <section class="tab-container">
        <section class="etf-container">
          ...
          <div class="nav">
            <india-stock-ex-nav :prominentListData="prominentListData" />
          </div>
          ...
          <div class="all-etf-ranking">
            ...
            <div class="more-box" @click="moreOpen(0, t('印度股'))">
...
import indiaStockExNav from "@/components/trade/india-stock-ex-nav/index.vue";
...
const handleIndexClick = (item, index) => {
  ...
  router.push(`/quotes/usStockIndexDetail?symbol=${item.symbol}&symbolType=INDIA-stocks`);
};
...
const itemClick = (item) => {
  router.push(
    `/quotes/usStockDetail?symbol=${item.symbol}&symbolType=INDIA-stocks&enName=${item.enName}&tabIndex=${props.tabActive}`
  );
};
//打开更多
const moreOpen = (index, name) => {
  router.push(`/quotes/UsStockMore?index=${index}&typName=${name}&symbolType=INDIA-stocks&tabIndex=${props.tabActive}`);
};
