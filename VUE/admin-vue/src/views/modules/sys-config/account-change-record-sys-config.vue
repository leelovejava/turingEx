    ...
    <template slot-scope="scope" slot="ndh">
      ...
      <template slot-scope="scope" slot="menu">
        <el-button
          ...
          icon="el-icon-edit"
          size="small"
          v-if="isAuth('sys-config:account-change-record-sys-config:operate')"
          @click.stop="moneylog(scope.row)"
          >{{scope.row.show*1 == 1?'隐藏前端显示操作':'展示前端显示操作'}}</el-button
        >
      </template>
    </avue-crud>
    <!-- 弹窗, 新增 / 修改 -->
...
<script>
import { tableOption } from "@/crud/sys/account-change-record";
