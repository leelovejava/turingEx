  ...
  <div class="mod-transport">
    ...
    >
      ...
      <template slot-scope="scope" slot="TransactionEngineStatus">
        <span 
        v-if="isAuth('jp-spots:operate')" class="speacButton" @click="TransactionEngineStatus(scope.row)"
          >查看</span
        >
      </template>
      ...
      <template slot-scope="scope" slot="menu">
        <el-button
          ...
          icon="el-icon-edit"
          size="small"
          v-if="isAuth('jp-spots:operate')&&scope.row.state=='submitted'"
...
<script>
import { tableOption } from "@/crud/us-spots/sell-order";
import AddOrUpdate from "./sell-order-add-or-update";
import parameters from "./sell-parameters";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getSymbol(page, params, done) {
      this.dataListLoading = true;
      this.$http({
        url: this.$http.adornUrl("/api/item!list.action?type=JP-stocks"),
        method: "get",
