    ...
    >
      ...
      <template slot-scope="scope" slot="menu">
        ...
        <el-button
          ...
          :loading="dataListLoading"
          v-if="isAuth('sys:user:root')"
          @click.stop="addkinHandle(scope.row.symbol,2)"
          >初始化K线图</el-button
        >
      </template>
    </avue-crud>
...
export default {
  ...
  methods: {
    ...
    getHkStocks() {
      this.$http({
        url: this.$http.adornUrl("/api/item!list.action?type=forex"),
        method: "get",
        params: this.$http.adornParams({}),
      }).then(({ data }) => {
    ...
    addkinHandle(symbol,n) {
      ...
      if (n == 1) {
        daType = {
          ...
          symbolType: "forex", //forex->外汇,commodities->大宗商品，指数/ETF->indices, A-stocks->A股, HK-stocks->港股.US-stocks->美股，cryptos->数字货币
        };
      } else {
        daType = {
          paraInitSymbol: symbol,
