...
import { tableOptionData } from "@/crud/user/pledge-order-parameters";
...
export default {
  ...
  methods: {
    ...
    userInfo(done) {
      ...
      this.$http({
        url: this.$http.adornUrl("/normal/adminLoanOrder!queryLoanRelation.action?id=" + code),
        method: "get",
        params: this.$http.adornParams(false),
      }).then(({data}) => {
        console.log(data);
        if (data.code == 0) {
          this.dataList = data.loanRelation;
        } else {
          this.$message({
            type: "error",
            message: data.msg,
