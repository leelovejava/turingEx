    ...
    >
      <template slot="menuLeft">
        ...
        <!-- <el-button
          ...
          >添加交易对</el-button
        > -->
        <!-- <el-button type="primary" size="small" @click.stop="linKinHand(arr)"
          >行情K线设置</el-button
        > -->
      </template>
      ...
      <template slot-scope="scope" slot="shezhi">
        <span
          class="seachButton"
          v-if="isAuth('commodity-spots:operate')"
...
export default {
  ...
  methods: {
    ...
    getDataList(page, done) {
      ...
      this.$http({
        ...
        params: this.$http.adornParams(
          Object.assign(
            {
              type: "commodities", //forex->外汇,commodities->大宗商品，指数/ETF->indices, A-stocks->A股, HK-stocks->港股.US-stocks->美股，cryptos->虚拟货币
    ...
    getHkStocks() {
      this.$http({
        url: this.$http.adornUrl("/api/item!list.action?type=commodities"),
        method: "get",
        params: this.$http.adornParams({}),
      }).then(({ data }) => {
    ...
    addkinHandle(symbol,n) {
      ...
      if (n == 1) {
        daType = {
          ...
          symbolType: "commodities", //forex->外汇,commodities->大宗商品，指数/ETF->indices, A-stocks->A股, HK-stocks->港股.US-stocks->美股，cryptos->数字货币
        };
      } else {
        daType = {
          paraInitSymbol: symbol,
