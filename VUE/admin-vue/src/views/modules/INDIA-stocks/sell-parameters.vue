...
import { tableOptionData } from "@/crud/etf-spots/buy-parameters";
...
export default {
  data() {
    return {
      ...
      type: "", //type:'forex', //forex->外汇,commodities->大宗商品，指数/ETF->indices, INDIA-stocks->A股, HK-stocks->港股.US-stocks->美股，cryptos->虚拟货币
      menuList: [],
      options: [],
      menuListTreeProps: {
        label: "name",
  ...
  methods: {
    ...
    getSymbol(row) {
      ...
      this.$http({
        url: this.$http.adornUrl("/exchangeApplyOrder/" + row.id),
        method: "get",
        params: this.$http.adornParams(Object.assign({}), false),
      }).then(({ data }) => {
        if (data.code == 0) {
    ...
    getDataList(page, params, done) {
      ...
      this.$http({
        ...
        params: this.$http.adornParams(
          Object.assign(
            {
              //type:this.type,//type:'forex', //forex->外汇,commodities->大宗商品，指数/ETF->indices, INDIA-stocks->A股, HK-stocks->港股.US-stocks->美股，cryptos->虚拟货币
