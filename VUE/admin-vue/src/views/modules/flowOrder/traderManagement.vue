    ...
    >
      ...
      <template slot="menuLeft">
        <el-button
          ...
          icon="el-icon-plus"
          size="small"
          v-if="isAuth('sys:traderManagement:save')"
      ...
      <template slot-scope="scope" slot="menu">
        ...
        <el-button
          ...
          icon="el-icon-edit"
          size="small"
          v-if="isAuth('sys:traderManagement:save') && scope.row.checked == 0"
          @click.stop="check(scope.row.id)"
          >审核</el-button
        >
        <el-button
          type="danger"
...
<script>
import { tableOption } from "@/crud/flowOrder/traderManagement";
...
export default {
  ...
  methods: {
    ...
    getDataList(page, done) {
      ...
      this.$http({
        url: this.$http.adornUrl("/normal/adminTrader!list.action"),
        method: "get",
    ...
    gotoHistory(){
      this.$router.push({
            path: "/flowOrder-traderHistoryManagement",
          });
    },
    // 新增 / 修改
    ...
    addClasscolor({ column, row }) {
      ...
      if (
        ...
        (column.property === "checked" && row.checked == 1)||
        (column.property === "profit" && row.profit > 0)||
        (column.property === "profit_ratio" && row.profit_ratio > 0)
      ) {
        return "green";
        ...
        (column.property === "checked" && row.checked == 0) ||  (column.property === "state" && row.state * 1 == 2)){
        return "yellow";
      }
    },
       // 审核
      ...
      this.$confirm(`确定进行[${id ? "审核通过" : "批量审核"}]操作?`, "提示", {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl(`/normal/adminTrader!check.action`),
            method: "get",
            params: this.$http.adornParams({
              uuid: id,
              check:1
    ...
    deleteHandle(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl(`/normal/adminTrader!delete.action`),
            method: "get",
            params: this.$http.adornParams({
              uuid: id,
            }),
