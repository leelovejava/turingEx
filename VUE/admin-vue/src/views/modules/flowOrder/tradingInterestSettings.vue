<template>
  <div class="mod-tradingInterestSettings">
    <avue-crud
      ref="crud"
      :page.sync="page"
      :data="dataList"
    ...
    >
      ...
      <template slot="menuLeft">
        <el-button
          ...
          icon="el-icon-plus"
          size="small"
          v-if="isAuth('sys:tradingInterestSettings:save')"
...
<script>
import { tableOption } from "@/crud/flowOrder/tradingInterestSettings";
import AddOrUpdate from "./tradingInterestSettings-add-or-update";
export default {
  data() {
    return {
      searchParams: {},
  ...
  methods: {
    ...
    getDataList(page, done) {
      ...
      this.$http({
        url: this.$http.adornUrl("/normal/adminTraderDaysSetting!list.action"),
        method: "get",
    ...
    deleteHandle(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl(`/normal/adminTraderDaysSetting!delete.action`),
            method: "get",
            params: this.$http.adornParams({
              uuid: id,
            }),
