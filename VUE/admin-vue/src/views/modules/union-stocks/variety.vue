  ...
  methods: {
    ...
    getAction(name) {
      this.$http({
        ...
        this.arr[name] = data.data.map((item, index) => {
    ...
    addkinHandle(symbol,n) {
      ...
      if (n == 1) {
        daType = {
          ...
          symbolType: this.stockName, //forex->外汇,commodities->大宗商品，指数/ETF->indices, A-stocks->A股, HK-stocks->港股.US-stocks->美股，cryptos->数字货币
        };
      } else {
        daType = {
          paraInitSymbol: symbol,
