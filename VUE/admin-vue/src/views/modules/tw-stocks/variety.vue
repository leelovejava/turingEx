    ...
    >
      ...
      <template slot-scope="scope" slot="menu">
        ...
        <el-button
          ...
          size="small"
          :loading="dataListLoading"
          v-if="isAuthisAuth('sys:user:root')"
...
export default {
  ...
  methods: {
    ...
    getDataList(page, done) {
      ...
      this.$http({
        ...
        params: this.$http.adornParams(
          Object.assign(
            {
              type: "TW-stocks", //forex->外汇,commodities->大宗商品，指数/ETF->indices, A-stocks->A股, TW-stocks->港股.US-stocks->美股，cryptos->虚拟货币
    ...
    getHkStocks() {
      this.$http({
        url: this.$http.adornUrl("/api/item!list.action?type=TW-stocks"),
        method: "get",
        params: this.$http.adornParams({}),
      }).then(({ data }) => {
    ...
    addkinHandle(symbol,n) {
      ...
      if (n == 1) {
        daType = {
          ...
          symbolType: "TW-stocks", //forex->外汇,commodities->大宗商品，指数/ETF->indices, A-stocks->A股, TW-stocks->港股.US-stocks->美股，cryptos->数字货币
        };
      } else {
        daType = {
          paraInitSymbol: symbol,
