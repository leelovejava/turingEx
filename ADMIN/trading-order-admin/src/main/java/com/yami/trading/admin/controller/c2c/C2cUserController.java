...
//        }
        ...
        if ("2".equals(c2c_user_type)) {
            ...
            this.saveLog(party, SecurityUtils.getSysUser().getUsername(), "操作：后台承兑商用户注册,ip[" + IPHelper.getIpAddr() + "]", Constants.LOG_CATEGORY_SECURITY);
        } else {
            throw new YamiShopBindException("承兑商类型不正确");
        }
        party.setC2cUserType(getInt(c2c_user_type));
        userService.updateById(party);
        C2cUser c2cUser = new C2cUser();
        c2cUser.setC2cManagerPartyId(c2c_manager_party_id);
        c2cUser.setC2cUserType(getInt(c2c_user_type));
        ...
        c2cUser.setDepositGiftRate(StringUtils.isEmptyString(deposit_gift_rate) ? 0.00 : Double.parseDouble(deposit_gift_rate));
        c2cUser.setThirtyDaysOrder(getInt(thirty_days_order_base));
        c2cUser.setThirtyDaysOrderRatio(StringUtils.isEmptyString(thirty_days_order_ratio) ? 0.00 : Double.parseDouble(thirty_days_order_ratio));
        ...
        String log = MessageFormat.format("ip:" + IPHelper.getIpAddr()
                        + ",新增承兑商,id:{0},承兑商类型:{1},承兑商CODE:{2},承兑商PARTY_ID:{3},承兑商昵称:{4},承兑商头像:{5},剩余保证金:{6},保证金:{7},"
...
//        paramBaseSet.setSecurityAppNoticeOpen(Integer.valueOf(security_app_notice_open_base).intValue());
        ...
        String logBase = MessageFormat.format("ip:" + IPHelper.getIpAddr()
                        + ",新增承兑商参数基础设置,id:{0},承兑商PARTY_ID:{1},"
...
//        }
        ...
        String log = MessageFormat.format("ip:" + IPHelper.getIpAddr()
                        + ",修改承兑商信息,id:{0},原承兑商类型:{1},原承兑商CODE:{2},原承兑商PARTY_ID:{3},原承兑商昵称:{4},原承兑商头像:{5},原剩余保证金:{6},原保证金:{7},"
...
//        c2cUser.setDepositGiftRate(Double.valueOf(deposit_gift_rate).doubleValue());
        ...
        String logBase = MessageFormat.format("ip:" + IPHelper.getIpAddr()
                        + ",修改承兑商参数基础设置信息,id:{0},原承兑商PARTY_ID:{1},"
