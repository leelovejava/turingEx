...
@RestController
@CrossOrigin
@RequestMapping("followmoneylog")
@Api(tags = "账号资金变动记录")
public class FollowMoneyLogController {
    ...
    public Result<Page<MoneyLogDto>> list(@RequestBody @Valid MoneylogListModel model){
        ...
        followMoneyLogService.pageMoneyLog(null,page,model.getRoleName(),model.getStartTime(),model.getEndTime(),model.getUserName(),
    ...
    public Result<String> list(@RequestBody @Valid MoneylogUpdateModel model){
        UpdateWrapper<FollowMoneyLog> updateWrapper = new UpdateWrapper<>();
        updateWrapper.eq("uuid", model.getUuid())
                        .set("`show`", model.getShow());
        followMoneyLogService.update(updateWrapper);
