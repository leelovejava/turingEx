...
public class ExchangeApplyOrderController {
    ...
    public Result close(@RequestBody @Valid CloseModel model) {
        ...
        sysUserOperService.saveLog(secUser, secUser.getUsername(), StrUtil.format("对 {} 执行撤销", model.getOrderNo()));
    ...
    public Result success(@RequestBody SuccessModel successModel) {
        ...
        if (ExchangeApplyOrder.OFFSET_OPEN.equals(order.getOffset())) {
            // 如果后台给的"成交数量" >= 订单实际的数量，则默认按全部数据成交
            if (successModel.getNumber() < order.getSymbolValue()) {
                order.setSuccessVolume(successModel.getNumber());
            }
        } else if (ExchangeApplyOrder.OFFSET_CLOSE.equals(order.getOffset())) {
            if (successModel.getNumber() < order.getVolume()) {
                order.setSuccessVolume(successModel.getNumber());
            }
        }
        exchangeApplyOrderHandleJob.handle(order, order.getPrice());
     ...
     * 交易所 转移账户冻结金额
     */
    @RequestMapping(value = "resetFreeze")
    @ApiOperation(("转移账户冻结金额"))
    public Result<Object> resetFreeze(ResetFreezeModel model) {
    ...
    public Result<Object> resetLock(ResetLockModel lockModel) {
        ...
        try {
            ...
            userService.saveResetLock(lockModel.getId(),
                    money_revise, lockModel.getLoginSafeword(), SecurityUtils.getSysUser().getUsername(), lockModel.getResetType(), IPHelper.getIpAddr(), lockModel.getCoinType());
