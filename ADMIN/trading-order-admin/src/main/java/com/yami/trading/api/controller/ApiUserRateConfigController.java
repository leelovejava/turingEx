...
@RestController
@CrossOrigin
@RequestMapping(value = "api/rate/userRateConfig")
public class ApiUserRateConfigController {
	@Autowired
	...
	public Result<UserRateConfigDTO> queryById(String id) {
		return Result.succeed ( userRateConfigWrapper.toDTO ( userRateConfigService.getById ( id ) ) );
	}
	...
	public  Result <String> save(@Valid @RequestBody UserRateConfigDTO userRateConfigDTO) {
        ...
        return Result.succeed ( null,"保存用户汇率管理成功" );
	}
	...
	public Result <String> delete(String ids) {
		...
		return Result.succeed ( null, "删除用户汇率管理成功");
	}
