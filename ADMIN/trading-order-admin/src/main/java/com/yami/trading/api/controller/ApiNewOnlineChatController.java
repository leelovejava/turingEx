...
public class ApiNewOnlineChatController {
    ...
    public Result list(ChatRecordReqModel recordReqModel) {
        ...
        List<OnlineChatMessage> onlinechatMessages = onlineChatMessageService.cacheGetList(messageId, 10, partyId, false);
        ...
        if (StringUtils.isNullOrEmpty(messageId)) {
            ...
            if (cacheMessageUser != null && cacheMessageUser.getUserUnreadmsg() > 0) {
                if (partyId.contains(".") || partyId.contains(":")) {
        ...
        for (OnlineChatMessage onlinechatMessage : onlinechatMessages) {
            ...
            if (onlinechatMessage != null && onlinechatMessage.getUuid() != null) {
                onlineChatMessageService.updateMessageRead(onlinechatMessage.getUuid(), "receive");
            }
            ...
            // 时区转换
            Date showCreateTime = DateTimeTools.transferToShowTime(onlinechatMessage.getCreateTime());
            chatRecordRespModel.setCreatetime(DateUtils.format(showCreateTime, "MM-dd HH:mm"));

            long showCreateTimestamp = DateTimeTools.transferShowTimeToClientTime(onlinechatMessage.getCreateTimeTs());
            chatRecordRespModel.setCreatetimeTs(showCreateTimestamp);
