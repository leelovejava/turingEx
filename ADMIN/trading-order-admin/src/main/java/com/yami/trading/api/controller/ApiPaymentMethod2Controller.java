...
public class ApiPaymentMethod2Controller {
    ...
    public Result list(HttpServletRequest request) {
        ...
        Map<String, C2cPaymentMethod> map =null;
        if (type.equals("2")) {
            map = this.c2cPaymentMethodService.getByC2CPartyId(SecurityUtils.getCurrentUserId());
        }
        else  if (type.equals("1")) {
            map = this.c2cPaymentMethodService.getByBankCardPartyId(SecurityUtils.getCurrentUserId());
        }
    ...
    public Result method_type(HttpServletRequest request) {
        ...
        for (String typeId : data.keySet()) {
            ...
            if (null != payType) {
                String payTypeLangKey = "pay.type." + typeId;
                C2cTranslate trans = this.c2cTranslateService.get(payTypeLangKey, language);
                if (null != trans) {
    ...
    public Result get(HttpServletRequest request) {
        ...
        method.setMethodTypeName(pmtMap.containsKey(method.getMethodType()+"") ? pmtMap.get(method.getMethodType()+"") : method.getMethodType() + "");
        if (StringUtils.isNotEmpty(method.getMethodImg())) {
        ...
        C2cPaymentMethod c2cPaymentMethod = this.c2cTranslateService.translatePm(method, language);

        return Result.succeed(c2cPaymentMethod);
    }
    ...
    public Result add(HttpServletRequest request) {
        ...
        int type = -1;
        if (StringUtils.isEmptyString(typeStr)){
            type = 2;
        } else {
     ...
     * 广告id
     */
    @RequestMapping(action + "getUserPaymentsByAd.action")
    public Object getUserPaymentsByAd(HttpServletRequest request) {
        String id = request.getParameter("id");
        String language = request.getParameter("language");
        if (StringUtils.isEmptyString(id)) {
            throw new BusinessException("广告id不正确");
        }
        ...
        String partyId = SecurityUtils.getCurrentUserId();
        List<C2cPaymentMethod> list = new ArrayList<C2cPaymentMethod>();
        Map<String, C2cPaymentMethod> map = this.c2cPaymentMethodService.getByPartyId(partyId);
        ...
        if (null == list || 0 == list.size()) {
            throw new BusinessException("用户支付方式未配置");
        }
        ...
        if (null == resList || 0 == resList.size()) {
            throw new BusinessException("用户广告支付方式未配置");
        }
