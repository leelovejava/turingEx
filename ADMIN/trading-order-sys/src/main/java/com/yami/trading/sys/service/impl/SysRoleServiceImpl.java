...
import com.yami.trading.sys.dao.SysRoleMapper;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
    ...
    public void updateRoleAndRoleMenu(SysRole role) {
        ...
        List<Long> ids = sysRoleMenuMapper.getIdByApp(roles, role.getAppType());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void deleteBatch(Long[] roleIds, String appType) {
        //删除角色
        sysRoleMapper.deleteBatch(roleIds);
        //删除角色与菜单关联
        List<Long> ids = sysRoleMenuMapper.getIdByApp(Arrays.asList(roleIds), appType);
        if (CollectionUtil.isNotEmpty(ids)) {
            sysRoleMenuMapper.deleteBatch(ids);
        }
        //删除角色与用户关联
