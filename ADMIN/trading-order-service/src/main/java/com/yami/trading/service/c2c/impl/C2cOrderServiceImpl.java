...
import com.yami.trading.common.util.C2cAdvertLock;
...
//		double symbolClose = symbolClose(c2cOrder.getSymbol());
        ...
        if (C2cOrder.DIRECTION_BUY.equals(c2cOrder.getDirection())) {
            ...
            moneyLog.setContentType(Constants.MONEYLOG_CONTENT_OTC_BUY);
    ...
    public void savePass(C2cOrder c2cOrder, String safeword, String operator_username) {
        ...
        this.saveLog(order_user.getUserName(), operator_username, "C2C订单放行", c2cOrder.getPartyId().toString());
    }
        ...
        if (C2cAdvert.DIRECTION_BUY.equals(c2cAdvert.getDirection())) {
            ...
            for (String id : methodMap.keySet()) {
                ...
                if (null != cpm && cpm.getMethodConfigId().equals(method.getMethodConfigId())) {
                    c2cOrder.setPaymentMethodId(method.getUuid().toString());
                    found = true;
                    break;
                }
            }
    ...
    public void saveC2COrderCancel(C2cOrder c2cOrder, String role) {
        ...
        if ("recharge".equals(c2cOrder.getDirection())) {
            ...
            moneyLog.setContentType(Constants.MONEYLOG_CONTENT_OTC_CANCEL);
    ...
    public void manualRelease(C2cOrder c2cOrder, String operator_username) {
        ...
        DecimalFormat df = new DecimalFormat("#.########");
        String logStr = "银行卡订单充值放行，订单号[" + c2cOrder.getOrderNo() + "]";
        ;
        if ("recharge".equals(c2cOrder.getDirection())) {
            // 充值
            ...
            this.userDataService.saveRechargeHandle(c2cOrder.getPartyId(), c2cOrder.getCoinAmount(), c2cOrder.getSymbol().toLowerCase());
        } else if ("withdraw".equals(c2cOrder.getDirection())) {
            // 提现

            this.userDataService.saveWithdrawHandle(c2cOrder.getPartyId(), c2cOrder.getCoinAmount(), 0d, "usdt");
            logStr = "银行卡订单提现放行，订单号[" + c2cOrder.getOrderNo() + "]";
        }
...
//        updateById(c2cOrder);
    ...
    public Page pagedQuery(int pageNo, int pageSize, List<String> direction, String state, String loginPartyId, boolean isC2cUser) {
        ...
        baseMapper.pagedQueryC2c(page, direction, state, loginPartyId, isC2cUser);
        // 金额默认保留2位
        DecimalFormat df = new DecimalFormat("#.##");
        // 币种默认保留8位
    ...
    public Page pagedQuery(long pageNo, long pageSize, String status, String orderNo, String userCode, String rolename,
                           String c2cUserCode, String c2cUserType, String c2cUserPartyCode, List<String> direction, List<String> children) {
        Page page = new Page(pageNo, pageSize);
        return baseMapper.pagedC2cQuery(page, status, orderNo, userCode, rolename, c2cUserCode, c2cUserType, c2cUserPartyCode, direction, children);
    }
...
//            }
    ...
    private void saveSell(C2cOrder c2cOrder, C2cAdvert c2cAdvert, double symbol_value, String remark) {
        ...
        if (C2cOrder.ORDER_TYPE_BY_AMOUNT.equals(c2cOrder.getOrderType())) {
            ...
            // 币种数量 = 支付金额/币种单价
            double coin_amount = Double.valueOf(df.format(Arith.div(c2cOrder.getAmount(), symbol_value, 10)));
            c2cOrder.setCoinAmount(coin_amount);
            c2cOrder.setAmountUsdt(Double.valueOf(df.format(Arith.mul(coin_amount, c2cAdvert.getSymbolClose(), 10))).doubleValue());
        } else {
            // 按币种数量支付
            c2cOrder.setAmount(Double.valueOf(df.format(Arith.mul(c2cOrder.getCoinAmount(), symbol_value))));
            c2cOrder.setAmountUsdt(Double.valueOf(df.format(Arith.mul(c2cOrder.getCoinAmount(), c2cAdvert.getSymbolClose(), 10))).doubleValue());
        }
    ...
    @Override
    public Page pagedBankCardOrderQuery(Page page, List<String> direction, String state, String userCode, String roleName, String orderNo, List<String> userIds) {
        return baseMapper.pagedBankCardOrderQuery(page, direction, state, userCode, roleName,
                orderNo, userIds);
    }

    @Override
    public Page pagedQuery(int pageNo, int pageSize, List<String> direction, String state, String loginPartyId) {
        Page page = new Page(pageNo, pageSize);
    ...
    public List<C2cPaymentMethod> getOrderPayments(String order_no, boolean c2cOrderFlag) {
        ...
        Map<String, C2cPaymentMethod> methodMap = null;
        if (c2cOrderFlag) {
            methodMap = this.c2cPaymentMethodService.getByC2CPartyId(c2cOrder.getPartyId());

        } else {
            methodMap = this.c2cPaymentMethodService.getByBankCardPartyId(c2cOrder.getPartyId());
        }
    ...
    public void saveOrderPay(String order_no, String safeword, String operator_username, String payment_method_id_order_pay) {
        ...
        this.tipService.deleteTip(c2cOrder.getUuid());
    }
    ...
    public void saveOpen(C2cOrder c2cOrder, User party) {
        //部分盘口无需充值方式
        if ("recharge".equals(c2cOrder.getDirection()) && StringUtils.isEmptyString(c2cOrder.getPaymentMethodId())) {
            Integer expireTime = c2cOrder.getExpireTime();
            ...
            c2cOrder.setMethodType(0);
            c2cOrder.setPaymentMethodId("");
...
//                throw new YamiShopBindException("支付方式不匹配官方");
    ...
    public List<C2cOrder> findAllNotComplete(String direction, String userId, Integer status) {
        return list(
                Wrappers.<C2cOrder>query().lambda()
                        .eq(C2cOrder::getDirection, direction)
                        .eq(C2cOrder::getPartyId, userId)
                        .eq(C2cOrder::getState, status));
    }
    ...
    private void saveWithdraw(C2cOrder c2cOrder) {
        ...
        this.walletService.update(c2cOrder.getPartyId(), Arith.sub(0, c2cOrder.getCoinAmount()));
        this.save(c2cOrder);
        this.tipService.saveTip(c2cOrder.getUuid(), TipConstants.BANK_CARD_ORDER, c2cOrder.getPartyId());
        // 保存 资金日志
        MoneyLog moneylog = new MoneyLog();
        moneylog.setCategory(Constants.MONEYLOG_CATEGORY_COIN);
        moneylog.setAmountBefore(new BigDecimal(amountBefore));
    ...
    @Override
    public C2cOrder findOrderNo(String orderNo) {
        return getOne(Wrappers.<C2cOrder>query().lambda().eq(C2cOrder::getOrderNo, orderNo));
    ...
    public Long getNofinishOrderCount(String partyId, String direction) {
        List<C2cOrder> c2cOrderList = list(Wrappers.<C2cOrder>query().lambda()
                .eq(C2cOrder::getPartyId, partyId).eq(C2cOrder::getDirection, direction)
                .in(C2cOrder::getState,
                        1, 2));
