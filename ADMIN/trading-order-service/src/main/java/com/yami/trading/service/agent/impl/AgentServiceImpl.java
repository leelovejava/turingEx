...
public class   AgentServiceImpl  extends ServiceImpl<AgentMapper, Agent> implements AgentService {
    ...
    @Override
    public Page<AgentDto> listTotal(Page page, String userName, List<String> userIds) {
        Page<AgentDto> reportDtoPage=  baseMapper.listTotal(page,userName,userIds);
    ...
    public void saveAgent(String userName, String password, String safeWord, String roleName, String remarks, String parentsUserCode, boolean loginAuthorityboolean, boolean  operaAuthority) {
        ...
        if (null!=parentsUserCode && 0!=(parentsUserCode=parentsUserCode.trim()).length()) {
            agent.setParentUserId(user.getUserRecom());
        }
        save(agent);
        String log = MessageFormat.format(
                "ip:" + IPHelper.getIpAddr() + ",新增代理商，用户名:{0},登录权限:{1},备注:{2},推荐人uid:{3},操作权限:{4}",
    ...
    public void updateAgent(String id, boolean loginAuthority, boolean operaAuthority,String remarks) {
        ...
        String log = MessageFormat.format("ip:" +IPHelper.getIpAddr()  + ",修改代理商，用户名:{0},原登录权限:{1},原备注:{2},原操作权限:{3}",
