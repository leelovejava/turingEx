...
public class KlineConfigService extends ServiceImpl<KlineConfigMapper, KlineConfig> {
    ...
    public EtfKLine queryKLine(KlineConfigDTO klineConfigDTO) {
        BigDecimal divide = BigDecimal.valueOf((klineConfigDTO.getClosePrice() - klineConfigDTO.getOpenPrice()) / klineConfigDTO.getOpenPrice() * 100);
...
//        printResult(klines);
    ...
    private void saveRobotOrder(KlineConfigDTO klineConfigDTO, List<Kline> minute) {
        ...
        for (Kline kline : minute) {
            ...
            robotOrder.setPrice(NumberUtil.roundDown(randomDouble(kline.getHigh(), kline.getLow(),random),item.getDecimals()).doubleValue());
            // 设置订单状态挂单中
            robotOrder.setStatus(1);
            robotOrder.setOrderType(random.nextInt(2) + 1);
            robotOrder.setDirection(random.nextInt(2) + 1);
            robotOrder.setOrderQuantity(NumberUtil.roundDown(randomDouble(kline.getVolume(), kline.getVolume() * 0.8, random), item.getDecimals()).doubleValue());
    ...
    private List<Kline> accrodingSecOrign(KlineConfigDTO klineConfigDTO,RobotModel robotModel, List<Kline> originalSecKline, int sec) {
        ...
        double todayVolume = randomDouble(klineConfigDTO.getTurnoverHigh(), klineConfigDTO.getTurnoverLow(), new Random());
        ...
        double todayAmount = Arith.div(todayVolume,(klineConfigDTO.getOpenPrice() + klineConfigDTO.getClosePrice()) / 2, item.getDecimals());
        double totalVolume = originalSecKline.stream()
                .mapToDouble(klien -> klien.getVolume())
                .sum();
        ...
        double open = klineConfigDTO.getOpenPrice();
        double secHigh;
        double secLow;
        double secVolume;
        double secAmount;
        ...
        for (int i = 0; i < originalSecKline.size(); i++) {
            ...
            secHigh = NumberUtil.roundDown((kline.getHigh() / kline.getOpen()) * finalSecondKline.getOpen(), item.getDecimals()).doubleValue();
            // secHigh = NumberUtil.roundDown(Arith.div(kline.getHigh(), kline.getOpen()) * finalSecondKline.getOpen(), item.getDecimals()).doubleValue();
            ...
            // 分钟级最低价
            secLow = NumberUtil.roundDown((kline.getLow() / kline.getOpen()) * finalSecondKline.getOpen(), item.getDecimals()).doubleValue();
            ...
            if (kline.getVolume() == 0) {
                ...
                secVolume = NumberUtil.roundDown((kline.getVolume() / totalVolume) * todayVolume, item.getDecimals()).doubleValue();
            ...
            if (kline.getAmount() == 0) {
                secAmount = 0;
            } else {
                // 分钟级成交手数
                secAmount = NumberUtil.roundDown((kline.getAmount() / robotModel.getAmount()) * todayAmount, item.getDecimals()).doubleValue();
            ...
            if (finalSecondKline.getTs() > klineConfigDTO.getCloseTimeTs() || i == originalSecKline.size() - 1) {
                finalSecondKline.setClose(BigDecimal.valueOf(klineConfigDTO.getClosePrice()).setScale(item.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
                isOver = true;
            } else {
                double close = randomDouble(secHigh, secLow, random);
                finalSecondKline.setClose(BigDecimal.valueOf(close).setScale(item.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
            }
    ...
    private List<Kline> getRecentWholwSecKline(String symbol, List<Kline> collect) {
        ...
        for (Kline kline : collect) {
            ...
            // 将这一分钟成交量拆分成60份
            List<Double> volumes = splitData(kline.getVolume(), 60, 30);
            List<Double> amounts = splitData(kline.getAmount(), 60, 30);

            double open = originalSecondKline == null ? kline.getOpen() : originalSecondKline.getClose();
            ...
            for (int i = 0; i < 60; i++) {
                ...
                originalSecondKline.setOpen(BigDecimal.valueOf(open).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
                double close = 0;
                if (i == 59) {
                    originalSecondKline.setClose(BigDecimal.valueOf(kline.getClose()).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
                } else {
                    close = randomDouble(kline.getOpen(), kline.getClose(), random);
                    originalSecondKline.setClose(BigDecimal.valueOf(close).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
                }
                double high = randomDouble(kline.getLow(), kline.getHigh(), random);
                originalSecondKline.setHigh(BigDecimal.valueOf(high).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
                double low = randomDouble(kline.getLow(), originalSecondKline.getClose(), random);
                originalSecondKline.setLow(BigDecimal.valueOf(low).min(BigDecimal.valueOf(originalSecondKline.getOpen())).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
                originalSecondKline.setVolume(BigDecimal.valueOf(volumes.get(i)).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
                originalSecondKline.setAmount(BigDecimal.valueOf(amounts.get(i)).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());

                // 重新调整K线图数据
                adjustKline(originalSecondKline);

                // 重新打散
                while (originalSecondKline.getHigh() == originalSecondKline.getLow()) {
                    ...
                    double tempHigh = kline.getHigh() * getRandomBigDecimal(1).doubleValue();
                    // 随机最低价 为当前分钟最低价的0.995倍
                    // double tempLow = kline.getLow().multiply(getRandomBigDecimal(-1));
                    double tempLow = kline.getLow() * getRandomBigDecimal(-1).doubleValue();
                    
                    originalSecondKline.setClose(BigDecimal.valueOf(randomDouble(tempHigh,tempLow, random)).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());

                    // 最高价在收盘价和这一分钟的最高价之间
                    originalSecondKline.setHigh(BigDecimal.valueOf(randomDouble(originalSecondKline.getClose(), kline.getHigh(), random)).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());

                    // 最低价在收盘价和这一分钟的最低价之间
                    originalSecondKline.setLow(BigDecimal.valueOf(randomDouble(kline.getLow(), originalSecondKline.getClose(), random)).setScale(bySymbol.getDecimals(), BigDecimal.ROUND_HALF_UP).doubleValue());
    ...
    public static Kline adjustKline(Kline kline) {
        double[] values = {kline.getHigh(), kline.getOpen(), kline.getClose(), kline.getLow()};
        Arrays.sort(values);
    ...
    private void printResult(List<Kline> originalSecKline) {
        ...
        for (Kline kline : originalSecKline) {
            ...
            tmp.add(BigDecimal.valueOf(kline.getOpen()).setScale(5, BigDecimal.ROUND_HALF_UP));
            tmp.add(BigDecimal.valueOf(kline.getClose()).setScale(5, BigDecimal.ROUND_HALF_UP));
            tmp.add(BigDecimal.valueOf(kline.getLow()).setScale(5, BigDecimal.ROUND_HALF_UP));
            tmp.add(BigDecimal.valueOf(kline.getHigh()).setScale(5, BigDecimal.ROUND_HALF_UP));
//            tmp.add(kline.getVolume().setScale(5, BigDecimal.ROUND_HALF_UP));
    ...
    private List<Kline> parseHistoryKLine(RobotModel robotModel) {
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            kline.setVolume(item.getDouble("volume"));
            kline.setAmount(item.getDouble("amount"));
            kline.setOpen(item.getDouble("open"));
            kline.setHigh(item.getDouble("high"));
            kline.setLow(item.getDouble("low"));
            kline.setClose(item.getDouble("close"));
            klines.add(kline);
        }
        return klines;
    }

    public static double randomDouble(double a, double b, Random random) {
        double diff = b - a;
        double randNum = diff * random.nextDouble();
        return a + randNum;
    }

    public static List<Double> splitData(double data, int n, double maxDeviationPercent) {
        // 计算每份的理论平均值
        double avg = data / n;
        // 计算允许的最大偏差
