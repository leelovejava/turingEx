...
public class OtcOnlineChatMessageServiceImpl implements OtcOnlineChatMessageService {
    ...
    private Logger logger = LoggerFactory.getLogger(OtcOnlineChatMessageServiceImpl.class);
...
//		start = start == 0 ? start : start + 1;// 空消息则表示首页，消息索引的后一条为起始
    ...
    public List<OtcOnlineChatMessage> cacheGetList(String messageId, int pageSize, String orderNo, String clicentType) {
        ...
        List<OtcOnlineChatMessage> result = new ArrayList<>(cache);
    ...
    public void saveSend(String partyId, String type, String content, String username, String orderNo) {
        ...
        if (null == username) {
            ...
            if (StringUtils.isNotEmpty(orderNo)) {
                C2cOrder c2cOrder = c2cOrderService.findOrderNo(orderNo);
                if (c2cOrder != null) {
                    String diretion = c2cOrder.getDirection();
                    if (diretion.equals("recharge") || diretion.equals("withdraw")) {
                       tipService.saveTip(onlineChatMessage.getUuid().toString(), TipConstants.BANK_CARD_ORDER_ONLINECHAT);
                    } else {
                        tipService.saveTip(onlineChatMessage.getUuid().toString(), TipConstants.OTCORDER_ONLINECHAT, onlineChatMessage.getPartyId());
                    }
                } else {
                    tipService.saveTip(onlineChatMessage.getUuid().toString(), TipConstants.OTCORDER_ONLINECHAT, onlineChatMessage.getPartyId());
                }
    ...
    public Map<String, Integer> unreadMsgs(List<String> orderNos) {
        return org.springframework.util.CollectionUtils.isEmpty(orderNos) ? Collections.EMPTY_MAP : orderNos.stream()
    ...
    public List<OtcOnlineChatMessage> cacheMessage(String key) {
        List<OtcOnlineChatMessage> otcOnlineChatMessages = cahce_chat.get(key);
        if (org.springframework.util.CollectionUtils.isEmpty(otcOnlineChatMessages)) {
            return otcOnlineChatMessages;
        }
        otcOnlineChatMessages.forEach(msg -> {
            String content = msg.getContent();
            content = UnicodeUtils.decodeUnicode(content);
            msg.setContent(content);
        });
    ...
    @Override
    public void updateMessageRead(String messageId, String orderNo) {
        OtcOnlineChatMessage otcOnlineChatMessage = getMessageById(messageId);
        if (null == otcOnlineChatMessage) return;
//		if(null != otcOnlineChatMessage && 0 == otcOnlineChatMessage.getIsRead()) { // 只更新未读消息
...
//			ApplicationUtil.executeUpdate(otcOnlineChatMessage);
        ...
        logger.info("OrderNo 11 -> " + otcOnlineChatMessage.getOrderNo());
        logger.info("uuid 11 -> " + otcOnlineChatMessage.getUuid());
        logger.info("PartyId 11 -> " + otcOnlineChatMessage.getPartyId());

        List<OtcOnlineChatMessage> list = cahce_chat.get(otcOnlineChatMessage.getOrderNo());
        list = list.stream().filter(elem -> null != elem).collect(Collectors.toList());
        if (null != list && !list.isEmpty()) {
            logger.info("OrderNo 22 -> " + otcOnlineChatMessage.getOrderNo());
            logger.info("uuid 22 -> " + otcOnlineChatMessage.getUuid());
            logger.info("PartyId 22 -> " + otcOnlineChatMessage.getPartyId());
//            int indexOf = list.indexOf(otcOnlineChatMessage);
            int indexOf = findOnlineChatMessage(otcOnlineChatMessage, list);
            if (indexOf > -1) {
                logger.info("OrderNo 33 -> " + otcOnlineChatMessage.getOrderNo());
                logger.info("uuid 33 -> " + otcOnlineChatMessage.getUuid());
                logger.info("PartyId 33 -> " + otcOnlineChatMessage.getPartyId());
                otcOnlineChatMessage.setIsRead(1);
                list.set(indexOf, otcOnlineChatMessage);
                updateMessageReadData(otcOnlineChatMessage);

                logger.info("otcOnlineChatMessage 33 -> " + otcOnlineChatMessage);
            }
        }

        cahce_chat.put(orderNo, list);
//		}

    }

    private void updateMessageReadData(OtcOnlineChatMessage otcOnlineChatMessage) {
        int count = jdbcTemplate.update("UPDATE t_otc_onlinechat_message SET IS_READ=? WHERE UUID=?"
                , otcOnlineChatMessage.getIsRead()
                , otcOnlineChatMessage.getUuid());

    }

    private int findOnlineChatMessage(OtcOnlineChatMessage onlineChatMessage, List<OtcOnlineChatMessage> list) {
        for (int i = 0; i < list.size(); i++) {
